  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-violet-50 dark:bg-violet-900/20 p-5 rounded-[24px] flex items-center gap-4 col-span-2 md:col-span-1 border border-violet-100 dark:border-violet-800/30">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase">অকুপেন্সি রেট</p>
            <p className="text-2xl font-black text-violet-700 dark:text-violet-400">{occupancy?.rate ?? 0}%</p>
            <p className="text-[10px] text-slate-400 dark:text-slate-500">{occupancy?.occupiedUnits ?? 0}/{occupancy?.totalUnits ?? 0} ইউনিট</p>
          </div>
        </div>

        {[
          { label: "মোট কালেকশন", value: `৳ ${(report?.totalCollection ?? 0).toLocaleString()}`, icon: <Wallet className="text-emerald-500" />, bg: "bg-emerald-50 dark:bg-emerald-900/20", border: "border-emerald-100 dark:border-emerald-800/30", text: "text-emerald-700 dark:text-emerald-400" },
          { label: "মোট বকেয়া", value: `৳ ${(report?.totalDue ?? 0).toLocaleString()}`, icon: <AlertCircle className="text-orange-500" />, bg: "bg-orange-50 dark:bg-orange-900/20", border: "border-orange-100 dark:border-orange-800/30", text: "text-orange-700 dark:text-orange-400" },
          { label: "নেট প্রফিট", value: `৳ ${(report?.netProfit ?? 0).toLocaleString()}`, icon: <TrendingUp className={report?.netProfit >= 0 ? "text-emerald-500" : "text-red-500"} />, bg: report?.netProfit >= 0 ? "bg-emerald-50 dark:bg-emerald-900/20" : "bg-red-50 dark:bg-red-900/20", border: report?.netProfit >= 0 ? "border-emerald-100 dark:border-emerald-800/30" : "border-red-100 dark:border-red-800/30", text: report?.netProfit >= 0 ? "text-emerald-700 dark:text-emerald-400" : "text-red-700 dark:text-red-400" },
        ].map((card) => (
          <div key={card.label} className={`${card.bg} ${card.border} border p-5 rounded-[24px] flex items-center gap-4`}>
            <div className="w-12 h-12 bg-white dark:bg-slate-800 rounded-2xl flex items-center justify-center shadow-sm">{card.icon}</div>
            <div>
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase">{card.label}</p>
      ...
      {/* Filter Bar */}
      <div className="bg-surface-container-lowest p-5 rounded-[28px] border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        {/* Presets */}
        <div className="flex items-center gap-2 flex-wrap">
          {presets.map(p => (
            <button key={p.id} onClick={() => applyPreset(p.id)}
              className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${activePreset === p.id ? "bg-primary text-white shadow-lg shadow-primary/20" : "bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700"}`}>
              {p.label}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div className="space-y-1.5">
            ...
            <input type="date" value={filters.startDate}
              ...
              className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none text-sm font-bold border border-slate-100 dark:border-slate-700 focus:border-primary/20 text-on-surface" />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <select value={filters.propertyId} onChange={(e) => setFilters({ ...filters, propertyId: e.target.value })}
              className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none text-sm font-bold border border-slate-100 dark:border-slate-700 focus:border-primary/20 min-w-[160px] text-on-surface">
      ...
      {isReportLoading ? (
        <div className="py-20 flex justify-center items-center gap-3">
          ...
          <span className="font-bold text-on-surface-variant">রিপোর্ট লোড হচ্ছে...</span>
        </div>
      ) : (
        <>
          {/* Charts Row */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-surface-container-lowest p-6 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm">
              <h3 className="text-lg font-bold text-on-surface mb-1">মাসিক আয় ট্রেন্ড (গত ১২ মাস)</h3>
              <p className="text-xs text-on-surface-variant mb-5">আয় বনাম খরচের তুলনামূলক বিশ্লেষণ</p>
              <div className="h-60">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={trendData ?? []}>
                    ...
                    <XAxis dataKey="month" axisLine={false} tickLine={false} tick={{ fontSize: 10, fontWeight: 700, fill: "currentColor" }} className="text-on-surface-variant" />
                    <YAxis hide />
                    <Tooltip
                      contentStyle={{ 
                        borderRadius: "1rem", 
                    ...
                    <Area type="monotone" dataKey="আয়" stroke="#7c3aed" strokeWidth={3} fill="url(#colorAy)" dot={false} />
                    <Area type="monotone" dataKey="খরচ" stroke="#ef4444" strokeWidth={3} fill="url(#colorKhoroch)" dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-surface-container-lowest p-6 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm">
              <h3 className="text-lg font-bold text-on-surface mb-1">প্রপার্টিওয়াইজ আয়</h3>
              <p className="text-xs text-on-surface-variant mb-5">সংগৃহীত ভাড়ার অনুপাত</p>
              {pieData.length > 0 ? (
                <div className="h-60">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={pieData} cx="50%" cy="50%" outerRadius={80} dataKey="value" stroke="none" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
          ...
          {barData.length > 0 && (
            <div className="bg-surface-container-lowest p-6 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm">
              <h3 className="text-lg font-bold text-on-surface mb-6">প্রপার্টিওয়াইজ আয় বনাম বকেয়া</h3>
              <div className="h-60">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={barData}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="currentColor" className="text-slate-200 dark:text-slate-800" />
                    <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 11, fontWeight: 700, fill: "currentColor" }} className="text-on-surface-variant" />
                    <YAxis hide />
                    <Tooltip
                      contentStyle={{ 
                        borderRadius: "1rem", 
          ...
          <div className="bg-surface-container-lowest rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
              <h3 className="text-lg font-bold text-on-surface">ট্রানজেকশন বিবরণ</h3>
              <span className="text-sm font-bold text-on-surface-variant">মোট: {report?.transactions?.length ?? 0}টি</span>
            </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-surface-container dark:bg-slate-800/50">
                      <th className="p-5 text-[10px] font-black text-on-surface-variant uppercase">ভাড়াটিয়া</th>
                      <th className="p-5 text-[10px] font-black text-on-surface-variant uppercase">প্রপার্টি / ইউনিট</th>
                      <th className="p-5 text-[10px] font-black text-on-surface-variant uppercase">বিলের মাস</th>
                      <th className="p-5 text-[10px] font-black text-on-surface-variant uppercase">পেমেন্ট তারিখ</th>
                      <th className="p-5 text-[10px] font-black text-on-surface-variant uppercase">মেথড / TxID</th>
                      <th className="p-5 text-[10px] font-black text-on-surface-variant uppercase text-right">পরিমাণ</th>
                      <th className="p-5 text-[10px] font-black text-on-surface-variant uppercase text-right">বকেয়া</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {report.transactions.map((txn: any) => (
                      <tr key={txn._id} className="hover:bg-surface-container/30 transition-all">
                        ...
                        <td className="p-5 text-sm text-on-surface-variant font-bold">
                          <div className="flex items-center gap-1.5">
                            <Home size={13} className="text-on-surface-variant/60" />
                        ...
                        <td className="p-5">
                          <div className="space-y-1">
                            <span className="px-3 py-1 bg-surface-container dark:bg-slate-800 text-on-surface-variant rounded-full text-[10px] font-black">
                              {txn.paymentMethod}
                            </span>
                            {txn.transactionId && (
                              <p className="text-[10px] text-on-surface-variant/80 font-bold">TxID: {txn.transactionId}</p>
                            )}
                          </div>
                        </td>
                        <td className="p-5 text-right">
                          <p className="font-black text-emerald-600 dark:text-emerald-500">৳ {txn.amount?.toLocaleString()}</p>
                        </td>
                        <td className="p-5 text-right">
                          {txn.invoice?.dueAmount > 0 ? (
                            <p className="font-bold text-orange-500 dark:text-orange-400 text-sm">৳ {txn.invoice?.dueAmount?.toLocaleString()}</p>
                          ) : (
                            <span className="px-2 py-0.5 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full text-[10px] font-black">পরিশোধিত</span>
                          )}
                        </td>
                      </tr>
                    ))}
